...
def testTimer():
    ...
    with LogCapture() as logCapture:
        ...
        def myFunction():
            ...
            """
            logging.info("Testing timer sleeping for 5s")
            time.sleep(5)

        myFunction()
        logger.info(myFunction.__doc__)
    # validate log capture
    # myFunction logs info "Testing timer sleeping for 5s
    # decoratorTypes info "def myFunction elapsed time=5.0
    # myFunction test for time decorator with 1 precision
    logCapture.check(
        ("root", "INFO", "Testing timer sleeping for 5s"),
        ("decoratorTypes", "INFO", "def myFunction elapsed time=5.0"),
        ("root", "INFO", "\n            myFunction test for time decorator with 1 precision\n            ")
    )
